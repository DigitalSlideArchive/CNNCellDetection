...
if __name__ == '__main__':
    PATH = 'tf_dataset/train.tfrecords'

    config_files = "/Users/ramA/2019/KWWork/ActualProject/luminoth-master/examples/sample_config.yml"
    from luminoth.utils.config import get_config
    config = get_config(config_files)

    # train_dataset = ObjectDetectionDataset(config)
    dataset = ObjectDetectionDataset(config)
    train_dataset = dataset()

    # train_image = train_dataset['image']
    train_image_raw = train_dataset['image']
    train_image = tf.stack([train_image_raw[:, :, 0], train_image_raw[:, :, 0],
                            train_image_raw[:, :, 0]], axis=2)
    ...
    with tf.Session() as sess:
        ...
        train_image_out, train_filename_out, train_bboxes_out = sess.run([train_image,
                                                                          train_filename,
                                                                          train_bboxes])
        ...
        import cv2
        cv2.imwrite('hello1.png', train_image_out)
        # print(train_image_out[0, :, :, :])

    # train_image_raw = train_dataset['image']
    # train_image = train_image_raw[:,:,0]
    # train_image = np.stack((train_image,) * 3, axis=-1)

    # train_image=tf.expand_dims(train_image, -1)
    # train_image[..., 1] = train_image_raw[:,:,0]
    # train_image[..., 2] = train_image_raw[:,:,0]

    # train_image=tf.concat([train_image_raw[:,:,0], train_image_raw[:,:,0],
    #     train_image_raw[:,:,0]], -1)
    # train_image=tf.concat([train_image_raw[:,:,0], train_image_raw[:,:,0],
    #     train_image_raw[:,:,0]], -1)
