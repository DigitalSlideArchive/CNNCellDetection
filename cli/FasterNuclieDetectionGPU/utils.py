...
def convert_preds_to_utilformat(nuclei_obj_props, pred_probs,
                                ignore_border_nuclei,
                                im_tile_size):

    annot_list = []
    analysis_list = []
    for i in range(nuclei_obj_props.shape[0]):
        obj_coords = nuclei_obj_props.loc[i]

        xmin = int(obj_coords[0])
        ymin = int(obj_coords[1])
        xmax = int(obj_coords[2])
        ymax = int(obj_coords[3])

        # =====================================================================

        if ignore_border_nuclei and \
                (xmin == 0 or xmax == im_tile_size or ymin == 0 or ymax == im_tile_size):
            continue
        ...
        tmp_annot_dict = {}
        tmp_annot_dict['type'] = "rectangle"
        tmp_annot_dict['center'] = [cx, cy, 0]
        tmp_annot_dict['width'] = width
        tmp_annot_dict['height'] = height
        tmp_annot_dict['rotation'] = 0
        tmp_annot_dict['fillColor'] = "rgba(0,0,0,0)"
        tmp_annot_dict['lineColor'] = color

        # ===========================================

        tmp_analysis_dict = {}

        tmp_analysis_dict['center'] = [cx, cy]
        tmp_analysis_dict['height'] = height
        tmp_analysis_dict['width'] = width
        tmp_analysis_dict['lineColor'] = color
        tmp_analysis_dict["pred_prob"] = tmp_prob

        annot_list.append(tmp_annot_dict)
        analysis_list.append(tmp_analysis_dict)

    return annot_list, analysis_list


def create_tile_nuclei_bbox_annotations(nuclei_obj_props, tile_info):
    ...
    for i in range(len(nuclei_obj_props)):
        ...
        cx = nuclei_obj_props[i]['center'][0]
        cy = nuclei_obj_props[i]['center'][1]
        width = nuclei_obj_props[i]['width'] + 1
        height = nuclei_obj_props[i]['height'] + 1

        class_color = nuclei_obj_props[i]['lineColor']
...
def create_tile_nuclei_annotations(im_nuclei_det_dict, tile_info, format):

    if format == 'bbox':

        return create_tile_nuclei_bbox_annotations(im_nuclei_det_dict,
                                                   tile_info)

    elif format == 'boundary':

        return create_tile_nuclei_boundary_annotations(im_nuclei_det_dict,
                                                       tile_info)
    else:
